//version 2, requiere el archivo run.js con el parametro web:
// node w2   Solo transpilación
// node w2 web  Transpilacion y servidor web 
...
async function analizar(){
  ...
  if (pila.length > 0) {
    ...
    //console.log("\n   Errores: ", errores)
    console.log("[JApp] Cambios de archivos No TSX....")
...
function tscw(){
  const TSC = spawn('npx', ['tsc', '-w'], {
        stdio: ['pipe', 'pipe', 'pipe'],//'inherit', // **CLAVE:** Transmite logs directamente a la consola principal
        shell: true      // Necesario para que comandos como 'nodemon' se ejecuten correctamente
    });

    TSC.stdout.on('data', (data) => {
        // Aquí puedes prefijar la salida antes de imprimirla
        //console.log(`[TSC]: ${data.toString().trim()}`);
        mistd("TSC", data.toString().trim())
    })

    return

  //levanta la auto transpilación
  exec('npx tsc -w &', (error, stdout, stderr) => {
    if (error) {
...
setTimeout(()=>{
  if (process.argv.indexOf("web") != -1){  
    //levanta el servidro web para dist
    //const WEB = spawn('nodemon run.js', [], {
    const WEB = spawn('node run.js', [], {
        stdio: ['pipe', 'pipe', 'pipe'],//'inherit', // **CLAVE:** Transmite logs directamente a la consola principal
        shell: true      // Necesario para que comandos como 'nodemon' se ejecuten correctamente
    });
    ...
    WEB.stdout.on('data', (data) => {
        ...
        mistd("WEB", data.toString().trim())
    })
  }

  if (process.argv.indexOf("mserver") != -1){
    const API = spawn('nodemon mserver/index.js', [], {
    //const API = spawn('node mserver/index.js', [], {
        stdio: ['pipe', 'pipe', 'pipe'],//'inherit', // **CLAVE:** Transmite logs directamente a la consola principal
        shell: true      // Necesario para que comandos como 'nodemon' se ejecuten correctamente
    });

    // Debes crear un evento que capture la salida
    API.stdout.on('data', (data) => {
        // Aquí puedes prefijar la salida antes de imprimirla
        mistd("API", data.toString().trim())
    });
    API.stdout.on('close', (data) => {
        // Aquí puedes prefijar la salida antes de imprimirla
        mistd("API", data.toString().trim())
    });
...
function mistd(hilo, mensaje){
  var block=false
  NoSTD.forEach(i=>{
    if ( mensaje.indexOf(i) != -1 ) block=true
  })

  if (!block) console.log(`[${hilo}] ${mensaje}`)
}